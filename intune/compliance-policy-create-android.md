...
- Uyumluluk ilkeleri hakkında daha fazla bilgi edinmek için bkz. [Cihaz uyumluluğu nedir](device-compliance.md).
- Uyumluluk ilkesi oluşturmadan önce ilgilenmeniz gereken önkoşullar hakkında bilgi edinmek için bkz. [Cihaz uyumluluğuyla çalışmaya başlama](device-compliance-get-started.md).
...
1. İlkeyi, sonra da **Atamalar**’ı seçin. Daha sonra **Azure Active Directory güvenlik gruplarını** seçebilir ve bunları ilkeye atayabilirsiniz.
...
- **Cihaz tehdit korumasının etkinleştirilmesini iste**: Lookout MTP çözümünden alınan risk değerlendirmesini uyumluluk koşulu olarak kullanmak için bu ayarı etkinleştirin. İzin verilen en yüksek tehdit düzeyini seçin. Bu düzey aşağıdakilerden biri olabilir:
  - **Hiçbiri (güvenli)**: Bu tehdit düzeyi en güvenli olanıdır. Cihazda herhangi bir tehdit olamayacağı anlamına gelir. Herhangi bir tehdit düzeyi algılanırsa cihaz, uyumlu değil olarak değerlendirilir.
  ...
  - **Yüksek**: Bu tehdit düzeyi en az güvenli olanıdır. Temelde bu, tüm tehdit düzeylerine izin verir. Bu seçenek, yalnızca raporlama amacıyla kullanıyorsanız kullanışlı olabilir.
...
- **Mobil cihazda şifreleme gerektir**: Cihazın kaynaklara bağlanma amacıyla şifrelenmesini gerektirmek için bunu **Evet** olarak ayarlayın. **Mobil cihazların kilidini açmak için parola iste** ayarını seçtiğinizde cihazlar şifrelenir.
