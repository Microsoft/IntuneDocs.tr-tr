---
title: "Intune ile Symantec PKCS sertifikaları yayımlama"
titlesuffix: Azure portal
description: "Symantec PKI Manager Web Hizmeti’nden Intune tarafından yönetilen cihazlara PKCS Sertifikaları yayımlamak için Intune Sertifika Bağlayıcı’yı kurun ve yapılandırın"
keywords: 
...
Intune Sertifika Bağlayıcı'yı Microsoft CA ve Symantec CA için kullanmak istiyorsanız, öncelikle Microsoft CA için Intune Sertifika Bağlayıcı doğrulamasını tamamlamanız ve ardından Symantec CA yapılandırması için bu adımları uygulamanız gerekir.  Bir Microsoft CA için Intune Sertifika Bağlayıcı'yı yapılandırmayla ilgili daha fazla ayrıntı için bkz. [Intune ile PKCS Sertifikalarını yapılandırın ve yönetin](certificates-scep-configure.md).
...
2. **Microsoft Intune** > **Cihaz Yapılandırması** > **Profiller** > **Profil oluşturma**’dan bir PKCS Sertifika profili oluşturun.
   ...
   > PKCS Sertifika Profilinin aşağıdaki parametreleri, Symantec CA'dan Intune Sertifika Bağlayıcısı aracılığıyla PKCS Sertifikaları yayınlamak için ekran görüntüsünde gösterildiği gibi aşağıdaki tabloda belirtilen değerlerle yapılandırılmalıdır. 
    ...
    | --- | --- | --- |
    | Sertifika yetkilisi | pki-ws.symauth.com | Bu değer sondaki eğik çizgi olmadan Symantec CA temel hizmet URL'si olmalıdır.  Symantec CA aboneliğiniz için doğru temel hizmet URL'si olup olmadığından emin değilseniz, Symantec Müşteri desteğine başvurun. <br><br> Bu URL yanlış ise, Intune Sertifika Bağlayıcı Symantec CA'sından PKCS sertifikasını yayımlamaz.| 
...
| Symantec Sağlayıcı - Symantec ilkesi "İşlem zaman aşımına uğradı" alınamadı | Intune Sertifika Bağlayıcı Symantec CA ile iletişim kurarken işlemi zaman aşımı hatası aldı. Bu hata devam ederse, bağlantı zaman aşımı değerini artırın ve yeniden deneyin. <br><br> Bağlantı zaman aşımını artırmak için: <br> 1. NDES Bağlayıcı bilgisayar 2’ye gidin. `%ProgramFiles%\Microsoft Intune\NDESConnectorSvc\NDESConnector.exe.config` Dosyayı Not Defteri’nde açın <br> 3. Aşağıdaki parametre için zaman aşımı değerini artırın <br><br> `CloudCAConnTimeoutInMilliseconds` <br><br> 4. Intune Bağlayıcıyı Hizmeti’ni yeniden başlatın <br> Sorun devam ederse, Symantec müşteri desteğine başvurun. |
