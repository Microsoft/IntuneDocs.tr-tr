...
- **Gerekli parola türü**: kuruluşunuzun gerektirdiği parola türünü seçin. Seçenekleriniz şunlardır:
  ...
  - **Sayısal**
  - **Alfasayısal**
- **Paroladaki alfasayısal olmayan karakter sayısı**: parolaya dahil olması gereken `#` veya `@` gibi simge karakterlerinin sayısını girin.
...
- **Parola veya kredi kartı bilgileri Için dokunma kimliği veya yüz kimliği kimlik doğrulaması ıste otomatik doldurma**: **gerekli**olarak ayarlandığında, parolaların veya kredi kartı bilgilerinin otomatik olarak Safari ve diğeri ile doldurulabilmesi için kullanıcıların TouchID veya çok yönlü kimliği kullanarak kimlik doğrulaması yapması gerekir gör. **Yapılandırılmadı** (varsayılan) ayarı kullanıcıların cihaz ayarlarında bu özelliği denetlemesine izin verir.
...
- **Siri komutları Için sunucu tarafında günlüğe kaydetme**: **devre dışı**olarak ayarlandığında, sunucu tarafı Siri günlüğü kapalıdır. Ayrıca, Siri sunucularında kullanıcı isteklerinin günlüğe kaydedilmesini da engelleyebilir. **Yapılandırılmadı** (varsayılan) Siri komutları sunucu tarafında günlüğe kaydedilir. Bu ayar engellenmekte olan veya yapılandırılmamış Siri ayarına bağlı değildir.

  Bu özellik şu platformlarda geçerlidir:  
  - iOS 12,2 ve üzeri

  > [!NOTE]
  > **Siri komutları Için sunucu tarafında günlüğe kaydetme** ayarı Apple tarafından kullanım dışıdır. Yaklaşan bir sürümde, bu ayar Intune konsolundan kaldırılır.
  >
  > Şu anda, ayar Managemenet profillerinde gösterilmese de, bu ayarın cihazlar üzerinde hiçbir etkisi yoktur. Bu ayarı herhangi bir ilkeden silmek için ilkeyi açın, küçük bir değişiklik yapın ve ilkeyi kaydedin. İlke güncellenir ve ayar cihazlardan silinir.
...
- **İşaretsiz e-posta etki alanları** > **e-posta etki alanı URL 'si**: listeye bir veya daha fazla URL Son kullanıcılar, girdiğiniz etki alanları dışındaki bir etki alanından e-posta aldığında bu e-posta iOS Mail uygulamasında güvenilmeyen olarak işaretlenir.
