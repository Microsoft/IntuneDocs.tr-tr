---
title: Microsoft Intune-Azure 'da cihaz profillerinin sorunlarını giderme | Microsoft Docs
...
## <a name="why-doesnt-a-user-get-a-new-profile-when-changing-a-password-or-passphrase-on-an-existing-wi-fi-profile"></a>Var olan bir Wi-Fi profilinde parola veya parolayı değiştirirken Kullanıcı neden yeni bir profil almaz?

Bir şirket Wi-Fi profili oluşturun, profili bir gruba dağıtın, parolayı değiştirin ve profili kaydedin. Profil değiştiğinde, bazı kullanıcılar yeni profili alamıyor olabilir.

Bu sorunu azaltmak için konuk Wi-Fi ayarlayın. Şirket Wi-Fi başarısız olursa, kullanıcılar Konuk Wi-Fi ' e bağlanabilir. Tüm otomatik bağlantı ayarlarını etkinleştirdiğinizden emin olun. Konuk Wi-Fi profilini tüm kullanıcılara dağıtın.

Bazı ek öneriler:  

- Bağlanmakta olduğunuz Wi-Fi ağı bir parola veya parola kullanıyorsa, Wi-Fi yönlendiricisine doğrudan bağlanabildiğinizden emin olun. İOS cihazından test edebilirsiniz.
- Wi-Fi uç noktasına (Wi-Fi yönlendiricisi) başarıyla bağlandıktan sonra SSID 'yi ve kullanılan kimlik bilgilerini (Bu değer parola veya parola) aklınızda bulabilirsiniz.
- Önceden paylaşılan anahtar alanına SSID ve kimlik bilgilerini (parola veya parola) girin. 
- Tercihen yalnızca BT ekibi olan sınırlı sayıda kullanıcısı olan bir test grubuna dağıtın. 
- İOS cihazınızı Intune 'a eşitleyin. Henüz kaydolmadıysanız kaydolun. 
- Aynı Wi-Fi uç noktasına (ilk adımda belirtildiği gibi) bağlanmayı sınayın.
- Daha büyük gruplara ve sonunda, kuruluşunuzdaki tüm beklenen kullanıcılara ulaşın. 
...
| Platform | Yenileme çevrimi|
| --- | --- |
| iOS | Her 8 saatte bir |
| macOS | Her 8 saatte bir |
| Android | Her 8 saatte bir |
| Cihaz olarak kaydedilen Windows 10 bilgisayarları | Her 8 saatte bir |
...
| Cihaz olarak kaydedilen Windows 10 bilgisayarları | 15 dakika boyunca 3 dakikada bir, sonra 2 saat boyunca 15 dakikada bir ve daha sonra 8 saatte bir | 
...
Herhangi bir zamanda, kullanıcılar Şirket Portalı uygulamayı açabilir ve ilkeyi veya profil güncelleştirmelerini anında denetlemek için cihazı eşitleyebilir.
...
## <a name="what-actions-cause-intune-to-immediately-send-a-notification-to-a-device"></a>Hangi eylemler Intune 'a cihaza anında bildirim gönderilmesine neden olur?
...
## <a name="if-multiple-policies-are-assigned-to-the-same-user-or-device-how-do-i-know-which-settings-gets-applied"></a>Aynı kullanıcıya veya cihaza birden çok ilke atanmışsa hangi ayarların uygulanacağını nasıl anlayabilirim?
...
- Bir yapılandırma ilkesi ayarı başka bir yapılandırma ilkesindeki bir ayarla çakışırsa, bu çakışma Intune 'da gösterilir. Bu çakışmaları el ile çözümleyin.

## <a name="what-happens-when-app-protection-policies-conflict-with-each-other-which-one-is-applied-to-the-app"></a>Uygulama koruma ilkeleri birbirleriyle çakışıyorsa ne olur? Uygulamaya hangisi uygulanır?
...
İki profil ayarı aynı olduğunda çakışmalar meydana gelir. Örneğin, Kopyala/Yapıştır ayarı dışında birbirinin aynı olan iki MAM ilkesi yapılandırdınız. Bu senaryoda, Kopyala/Yapıştır ayarı en kısıtlayıcı değer olarak ayarlanır, ancak ayarların geri kalanı yapılandırıldığı gibi uygulanır.

Bir ilke uygulamaya dağıtılır ve devreye girer. İkinci bir ilke dağıtılır. Bu senaryoda, ilk ilke öncelik kazanır ve uygulanır. İkinci ilke bir çakışma gösterir. Her ikisi de aynı anda uygulanırsa, önceki ilke olmadığında, her ikisi de çakışmadır. Çakışan tüm ayarlar en kısıtlayıcı değerlere ayarlanır.

## <a name="what-happens-when-ios-custom-policies-conflict"></a>İOS özel ilkeleri çakıştığında ne olur?

Intune, Apple yapılandırma dosyalarının veya özel bir Open Mobile Alliance Tekdüzen Kaynak tanımlayıcısı (OMA-URI) ilkesinin yükünü değerlendirmez. Yalnızca teslim mekanizması olarak görev yapar.
...
- Diğer tüm profil türleri:  
  ...
  - **Windows Phone 8,1 cihazları**: aşağıdaki ayarlar kaldırılır:  
    ...
    - Cihaz temizlenmeden önce izin verilecek yinelenen başarısız oturum açma sayısı
    - Parola istenmeden önce geçen işlem yapılmayan dakika sayısı
    - Gerekli parola türü – minimum karakter kümesi sayısı
    - Kameraya izin ver
    - Mobil cihazda şifreleme gerektir
    - Çıkarılabilir depolamaya izin ver
    - Web tarayıcısına izin ver
    - Uygulama depolamaya izin ver
    - Ekran yakalamaya izin ver
    - Coğrafi konuma izin ver
    - Microsoft hesabı izin ver
    - Kopyalama ve yapıştırmaya izin ver
    - Wi-Fi paylaşımına izin ver
    - Ücretsiz Wi-Fi etkin noktalarına otomatik bağlantıya izin ver
    - Wi-Fi etkin nokta raporlamaya izin ver
...
## <a name="i-changed-a-device-restriction-profile-but-the-changes-havent-taken-effect"></a>Bir cihaz kısıtlama profilini değiştirdim, ancak değişiklikler devreye alınmadı
