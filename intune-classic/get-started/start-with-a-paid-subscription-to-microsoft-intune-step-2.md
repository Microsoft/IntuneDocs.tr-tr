...
4. **Etki alanını doğrula** iletişim kutusu açılarak, DNS barındırma sağlayıcınızdaki TXT kaydını oluşturmak için değerleri verir.
    ...
    > [!TIP]
    > DNS barındırma sağlayıcınızda değişiklik yaparken DNS diğer adı oluşturduğunuzdan emin olun /intune-classic/deploy-use/set-up-windows-phone-management-with-microsoft-intune).
