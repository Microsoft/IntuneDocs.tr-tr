...
Windows Phone mobil cihazları Intune’la yönetebilmek için, önce yönetim gereksinimlerini ayarlamanız gerekir. [DNS CNAME oluşturmak](set-up-windows-phone-management-with-microsoft-intune.md), kullanıcıların Intune şirket portalına bağlanmalarına yardımcı olur. Windows Phone 8.0, cihazlar ve Intune arasında şifrelenmiş bir IP bağlantısı kurmak için Symantec sertifikası gerektirir. İş kolu uygulamalarını imzalamak için de bir sertifika gereklidir.
...
1.  **Intune’u ayarlama**
    Henüz yapmadıysanız mobil cihaz yönetimine hazırlanmak için [mobil cihaz yönetimi yetkilisini](https://technet.microsoft.com/library/mt346013.aspx) **Microsoft Intune** olarak ayarlayın ve MDM’yi ayarlayın.
...
2.  **Kayıt sunucusu adresi için bir DNS diğer adı ayarlama** (isteğe bağlı)
    ...
    2.  **Doğrulanmış etki alanı adı belirtin** kutusuna şirket web sitesinin doğrulanan etki alanının URL'sini yazın ve ardından **Otomatik Algılamayı Sına**'ya tıklayın..
    ...
    3.  Şirketinizin etki alanı için **CNAME** DNS kaynak kayıtları oluşturun. CNAME kaynak kayıtları, aşağıdaki bilgileri içermelidir:
        ...
        |Ana Bilgisayar Adı|Şunu gösterir:|TTL|
        |-------------|-------------|-------|
        |enterpriseenrollment.company_domain.com|enterpriseenrollment.manage.microsoft.com |1 Saat|
        ...
        -   `manage.microsoft.com` – E-postanın etki alanı adından etki alanını tanıma ile Intune hizmetine yeniden yönlendirmeyi destekler.
...
3.  **Uygulama imzalamayı desteklemek için sertifika yönetimi**
    [Windows Phone Mağazası’na erişmeyecek ve/veya iş kolu uygulamalarına ihtiyacı olan Windows Phone 8.0 ve Windows Phone 8.1 için gereklidir.]
    ...
    5.  **İmzalama sertifikasını dışarı aktarma**
        ...
        **Sertifika Dışarı Aktarma Sihirbazı**’nda **Evet, özel anahtarı dışarı aktar**’ı seçin ve **İleri**’ye tıklayın. **Kişisel Bilgi Değişimi –PKCS #12 (.PFX)** öğesini seçin ve **Mümkünse sertifika yolundaki tüm sertifikaları ekle**’yi işaretleyin. Sihirbazı tamamlayın. Daha fazla bilgi için bkz. [Özel Anahtara Sahip Bir Sertifikayı Dışarı Aktarma](http://go.microsoft.com/fwlink/?LinkID=203031)..
    ...
    6.  **Şirket Portalı uygulamasını indirip imzalama**
        ...
        1.  **Şirket Portalı'nı indirme**
            İndirme Merkezi'nden [Windows Phone için Intune Şirket Portalı](http://go.microsoft.com/fwlink/?LinkId=268440) 'nı indirin. Varsayılan yükleme konumu: `C:\Program Files (x86)\Microsoft Corporation\Windows Intune Company Portal for Windows Phone`.

        2.  **Windows Phone 8.0 SDK’sını indirme**
            [Windows Phone SDK'sını](http://go.microsoft.com/fwlink/?LinkId=615570) indirin..

        3.  **Şirket Portalı uygulamasını kodla imzalama**
            Şirket portalını Symantec sertifikasından oluşturduğunuz .pfx dosyası ile imzalamak için SDK ile indirilen XAPSignTool uygulamasını kullanın. Daha fazla bilgi için bkz. [XapSignTool kullanarak şirket uygulaması imzalama](http://go.microsoft.com/fwlink/?LinkID=280195)..

    7.  **Şirket Portalı uygulamasını Intune’a yükleme**
        İmzalanmış Şirket Portalı uygulama dosyasını ve kod imzalama sertifikanızı karşıya yükleyerek uygulamayı son kullanıcılarınızın kullanımına sunun.
...
4.  **Kullanıcılara şirket portalı ile şirket kaynaklarına nasıl erişeceklerini anlatın**
...
2.  **Windows Phone SDK'sını indirme**
    [Windows Phone SDK 8.0](http://go.microsoft.com/fwlink/?LinkId=615570)’ı indirin (http://go.microsoft.com/fwlink/?LinkId=268439) ve SDK’yı bilgisayarınıza yükleyin. Bu SDK, bir uygulama kaydı belirteci oluşturmak için gereklidir.

3.  **AETX dosyası oluşturma**
    Windows Phone SDK 8.0’ın parçası olan AETGenerator.exe dosyasını kullanarak Symantec PFX dosyasından bir uygulama kaydı belirteci (.aetx) dosyası oluşturun. AETX dosyasının nasıl oluşturulacağına ilişkin yönergeler için bkz. [Windows Phone için uygulama kaydı belirteci oluşturma](https://msdn.microsoft.com/library/windows/apps/jj735576.aspx)

4.  **Windows 81 için Windows SDK’sını indirme**
    [Windows Phone SDK’sını](http://go.microsoft.com/fwlink/?LinkId=613525) indirip yükleyin (http://go.microsoft.com/fwlink/?LinkId=613525). Şirket Portalı uygulamasına dahil edilen PowerShell betiğinin varsayılan yükleme konumunu ( `${env:ProgramFiles(x86)}\Windows Kits\8.1`) kullandığını unutmayın. Başka bir yere yüklerseniz, bir cmdlet parametresinde konumu eklemeniz gerekir.

5.  **PowerShell kullanarak uygulamayı kodla imzalama**
    Yönetici olarak, Windows SDK ve Symantec Kurumsal Mobil Kod İmzalama Sertifikası yüklenmiş ana bilgisayarda **Windows PowerShell**’i açın, Sign-WinPhoneCompanyPortal.ps1 dosyasına gidin ve betiği çalıştırın.
